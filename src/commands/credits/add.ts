...
export const data = new SlashCommandSubcommandBuilder()
    .setName('add')
    .setDescription('Add credits to member\'s balance.')
...
export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    if (interaction.user.id !== target.discordId) await bot.users.send(target.discordId, {
        embeds: [
            bot.embed
                ...
                .setDescription(`Your credit balance has been increased by ${amount} credit(s). If you have any questions about this, please contact the ACSD administration.`)
                .setFields(
    ...
    await interaction.editReply({
        embeds: [
            bot.embeds.success
                .setDescription(`Successfully added ${amount} credit(s) to ${target.robloxUsername}'s balance${extra}.`)
                .setFields(
                    { name: 'Before:', value: `${credits?.amount ?? 0}`, inline: true },
                    { name: 'After:', value: `${(credits?.amount ?? 0) + amount}`, inline: true },
                    { name: 'Reason:', value: reason },
